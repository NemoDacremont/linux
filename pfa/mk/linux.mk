...
llvm_prefix=${PWD}/${BUILD_DIR}/llvm-19.1.4-rust-1.83.0-x86_64
...
${BUILD_DIR}/llvm-19.1.4-rust-1.83.0-x86_64.tar.gz:
	mkdir -p $(dir $@)
	wget -O "$@" https://mirrors.edge.kernel.org/pub/tools/llvm/rust/files/llvm-19.1.4-rust-1.83.0-x86_64.tar.gz
	[ -f "$@" ] || (echo -E '(ERROR) wget failed, check your connexion' 1>&2 ; exit 1)

${BUILD_DIR}/llvm-19.1.4-rust-1.83.0-x86_64: ${BUILD_DIR}/llvm-19.1.4-rust-1.83.0-x86_64.tar.gz
	tar -xf $< -C $(dir $@)
	touch $@
...
${LINUX_PATH}/.config:
	...
	echo 'CONFIG_SAMPLE_RUST_DRIVER_PCI=m' >> $@
	echo 'CONFIG_RUST_OVERFLOW_CHECKS=y' >> $@

${BUILD_DIR}/.bindgen.installed: ${BUILD_DIR}/llvm-19.1.4-rust-1.83.0-x86_64
...
${LINUX_PATH}/vmlinux: ${BUILD_DIR}/.bindgen.installed ${BUILD_DIR}/llvm-19.1.4-rust-1.83.0-x86_64 ${LINUX_PATH}/.config
